...
len_weight <- function(weight, length, grouping_var = NULL, data) {
  ...
  len_weight_mod <- function(data) {
    ...
    coefs <- data.frame(
      ...
      Std.Error.range = paste(
        "(",
        round(exp(coef(m)[1] - err[1]) / 10^expon, 3),
        "-",
        round(exp(coef(m)[1] + err[1]) / 10^expon, 3),
        ")"
      ),
      ...
      len.range = paste(
        ...
        "-",
        max(data$length[which(data$weight > 0)], na.rm = T)
      )
    )
    len_range <- with(
      data,
      seq(min(length, na.rm = T), max(length, na.rm = T), 1)
    )

    # Get mean, confidence and prediction intervals, adjusting for bias in log transform
    pred <- cbind(
      data.frame(length = len_range),
      exp(
        predict(
          m,
          newdata = data.frame(length = len_range),
          interval = "confidence"
        ) +
          summary(m)$sigma^2 / 2
      )
